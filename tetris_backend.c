...
enum result gametask(enum gameid gid)
{
  ...
  if( game_status[game1] && game_status[game2] && (curr_line_cnt >= LINECLR_SIM) ) send_lines(gid, curr_line_cnt);
  ...
  /* to check game over situation */
    ...
    if( game_status[gid] == off ) game->num_of_tet = 0;
...
{ 
  ...
  if(toggle) {
    ...
    switch(key_in) {
      /* key_in for game1(Left side of keypad) */
      case 'D'      : 
      case 'd'      : move_tetromino(left, game1); break;
      case 'G'      : 
      case 'g'      : move_tetromino(right, game1); break;
      case 'F'      :
      case 'f'      : move_tetromino(down, game1); break;
      case 'R'      :
      case 'r'      : move_tetromino(rotate, game1); break;
      case 'A'      :
      case 'a'      : drop_tetromino(game1); break;
      case 'S'      :
      case 's'      : game = parameter_db(game1); if(game->bonus_item > 0) use_bonus_item(game1); break;
      case KEY_F(1) : init_board(game1); break;    // erase all tetromino on Game1 board
            
      /* key_in for game2(Right side of keypad) */
      case KEY_LEFT      : move_tetromino(left, game2); break;
      case KEY_RIGHT     : move_tetromino(right, game2); break;
      case KEY_DOWN      : move_tetromino(down, game2); break;
      case KEY_UP        : move_tetromino(rotate, game2); break;
      case ','           : 
      case '<'           : drop_tetromino(game2); break;
      case '.'           : 
      case '>'           : game = parameter_db(game2); if(game->bonus_item > 0) use_bonus_item(game2); break;
      case KEY_F(12)     : init_board(game2); break;    // erase all tetromino on Game2 board
      ...
      case KEY_F(4)     : if (gameover == on) next_game(continuemode); break;
      case KEY_F(5)     : if (gameover == on) next_game(newmode); break;
      case KEY_F(6)     : if (gameover == on) on_game = off; break;

      default       : break;
    }

/* #### Not used below code, because of performance issue ####
...
enum result send_lines(enum gameid gid, int curr_line_cnt)
{
  ...
  if(gid == game1) other_player_id = game2;
  else if(gid == game2) other_player_id = game1;
  else return fail;
...
enum result control_game_level(enum gameid gid)
{
  ...
  if(game->score - game->bonus_score > BONUS_BASE) {
    ...
    if(game->bonus_item < MAX_BONUS_ITEM) game->bonus_item++;
  }  
...
enum result next_stage(enum gameid gid)
{
  ...
  if(game->stage < TOTAL_STAGE) game->stage++;
  game->curr_tetromino = I_TETROMINO;
  game->next_tetromino = show_next_tetromino(game->wnd_x, game->wnd_y - TITLE_HEIGHT, gid);  
  game->rot_degree = DEG_90;
  game->line_cnt = 0;
